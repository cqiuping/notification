...
import {
  IonicPage, MenuController, NavController, NavParams, ToastController,
  ViewController
} from 'ionic-angular';
...
export class FaultPage extends BaseUI {
  faultLists;
  alertLevels;
  faultStates;
  faultTypes;
  ...
  getFaultTabele() {
    ...
    .subscribe(
      res => {
        this.faultLists = res["responseParams"];
        console.log(this.faultLists);
      },
      err => {
        super.showToast(this.toast, "获取工单错误:" + err.message);
      }
    )
  }
  ...
  getRecvUsername(param: FaultParam) {
    ...
    .subscribe(
      res => {
        this.initRecvUsernames(res["responseParams"]);
      },
      err => {
        super.showToast(this.toast, "获取认领用户有误:" + err.message);
      }
    )
  }
